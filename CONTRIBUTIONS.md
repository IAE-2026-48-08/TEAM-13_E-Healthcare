...
- Mengatur Docker Compose dan komunikasi internal antar microservice
- Menambahkan beberapa PHP CLI server worker untuk mencegah deadlock pada validasi silang antar layanan
...
- Mengimplementasikan validasi pasien ke Data Pasien Service dan validasi appointment ke Appointment Service secara internal (REST, container ke container) sebelum resep disimpan
- Mengubah API Key Farmasi agar dibaca dari environment Docker, pakai `KEY-MHS-157`
...
- Memperbaiki konfigurasi `.env` Farmasi Service yang awalnya nggak lengkap (APP_KEY dan koneksi database belum ada), yang sebelumnya bikin 500 error
- Memperbaiki Dockerfile Data Pasien Service dan Appointment Service yang awalnya nggak menjalankan migrasi database otomatis, jadi setiap kali container di-rebuild dari nol selalu error database belum ada
- Menambahkan seeder otomatis (data pasien dan appointment dummy) supaya testing nggak perlu insert data manual setiap rebuild
- Ikut menemukan dan memperbaiki root cause masalah `patient_check` dan `appointment_check` yang gagal random meski environment variable sudah benar. Setelah ditelusuri lewat curl langsung antar container dan cek log, ternyata penyebabnya `php artisan serve` nggak konsisten meneruskan environment variable Docker ke proses yang menangani request HTTP. Solusinya ganti command server jadi `php -S 0.0.0.0:8000 public/index.php` di seluruh service
...
- Pengujian komunikasi antar microservice
