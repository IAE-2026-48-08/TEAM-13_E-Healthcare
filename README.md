...
| ------------------- | ------------ | -------------------------------------------- |
| Faiza Clara Vimanda | 102022400300 | Appointment Service dan integrasi end-to-end |
| Tabitha Sirait      | 102022400238 | Data Pasien Service                          |
...
Semua request dilakukan melalui API Gateway. Port internal `8000` hanya digunakan untuk komunikasi antarmicroservice di dalam Docker network.
...
`patient_name` tidak perlu dikirim karena diperoleh dari Data Pasien Service berdasarkan `patient_id`.
...
```powershell
Invoke-RestMethod `
  -Uri "http://localhost:8081/api/v1/patients" `
  -Headers @{"X-IAE-KEY"="KEY-MHS-279"}
```
...
```powershell
Invoke-RestMethod `
  -Uri "http://localhost:8081/api/v1/appointments" `
  -Headers @{"X-IAE-KEY"="KEY-MHS-390"}
```
...
```powershell
Invoke-RestMethod `
  -Uri "http://localhost:8081/api/v1/pharmacy" `
  -Headers @{"X-IAE-KEY"="KEY-MHS-157"}
```
...
Setiap microservice memiliki API Key yang berbeda. Request dengan API Key salah atau tanpa API Key menghasilkan:
...
Digunakan untuk memperoleh token JWT M2M menggunakan identitas Appointment Service.

### SOAP Audit

Mencatat transaksi appointment dan menghasilkan receipt number, contohnya:

```text
IAE-LOG-2026-693C042D
```

### RabbitMQ Publisher

Mengirim event appointment dengan konfigurasi:
...
Data tetap tersedia setelah container dihentikan atau dibangun ulang selama volume tidak dihapus.
...
* Integrasi end-to-end Data Pasien, Appointment, dan Farmasi berhasil.

## Dokumentasi Tambahan

* [`CONTRIBUTIONS.md`](CONTRIBUTIONS.md) — pembagian kontribusi anggota.
* [`CHANGELOG.md`](CHANGELOG.md) — riwayat perubahan proyek.
* [`PROMPTING_LOG.md`](PROMPTING_LOG.md) — dokumentasi penggunaan AI selama pengembangan.
